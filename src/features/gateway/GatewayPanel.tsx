...
function Result({ result, error }: { result: unknown; error?: string }) { const { t } = useI18n(); return error ? <div className="error-box" role="alert">{error}</div> : result !== undefined ? <details className="evidence"><summary>{t("operationResult")}</summary><pre>{JSON.stringify(result, null, 2)}</pre></details> : null; }
