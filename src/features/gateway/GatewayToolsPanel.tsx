...
export function GatewayToolsPanel({ section, profileId, workspaceId }: { section: string; profileId: string; workspaceId?: string }) {
 ...
 return <section className="panel"><div className="panel-heading"><div><h2>{section}</h2><p>{t("toolsScopedHint").replace("{profile}",profileId)}{workspaceId?t("andWorkspace").replace("{workspace}",workspaceId):""}</p></div></div><div className="resource-form">{definition.fields.map(field=><label key={field}>{field}<input value={values[field]??""} onChange={event=>set(field,event.target.value)} autoComplete="off"/></label>)}</div><div className="action-row">{definition.actions.map(action=><button key={action} disabled={busy!==undefined} onClick={()=>void run(action)}>{busy===action?t("working"):action}</button>)}</div>{error&&<div className="error-box" role="alert">{error}</div>}<Evidence result={result} summary={t("gatewayResponse")} open /></section>;
}
