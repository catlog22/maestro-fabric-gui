...
export function GatewayToolsPanel({ section, title, profileId, workspaceId }: { section: string; title?: string; profileId: string; workspaceId?: string }) {
 const { t }=useI18n(); const definition=definitions[section as Section]; const [values,setValues]=useState<Record<string,string>>({}); const [busy,setBusy]=useState<string>(); const [result,setResult]=useState<unknown>(); const [error,setError]=useState<string>(); if(!definition)return null; const tool="tool" in definition ? definition.tool : section.toLowerCase();
 const set=(key:string,value:string)=>setValues(current=>({...current,[key]:value}));
 async function run(action:string){if(highRisk.has(action)&&!window.confirm(t("confirmAction").replace("{action}",`${tool}.${action}`)))return;setBusy(action);setError(undefined);try{const args:Record<string,unknown>={action,...Object.fromEntries(Object.entries(values).filter(([,value])=>value.length>0)),...(workspaceId?{workspaceId}:{} )};setResult(await desktopApi.callGateway(profileId,tool,args));}catch(value){setError(value instanceof Error?value.message:String(value));}finally{setBusy(undefined);}}
 return <section className="panel"><div className="panel-heading"><div><h2>{title ?? section}</h2><p>{t("toolsScopedHint").replace("{profile}",profileId)}{workspaceId?t("andWorkspace").replace("{workspace}",workspaceId):""}</p></div></div><div className="resource-form">{definition.fields.map(field=><label key={field}>{field}<input value={values[field]??""} onChange={event=>set(field,event.target.value)} autoComplete="off"/></label>)}</div><div className="action-row">{definition.actions.map(action=><button key={action} disabled={busy!==undefined} onClick={()=>void run(action)}>{busy===action?t("working"):action}</button>)}</div>{error&&<div className="error-box" role="alert">{error}</div>}<Evidence result={result} summary={t("gatewayResponse")} open /></section>;
}
